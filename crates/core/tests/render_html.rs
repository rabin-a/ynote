...
#[test]
fn preview_edit_keeps_original_image_url() {
    // In edit mode the WYSIWYG serializer must restore the original image path,
    // so the renderer keeps it in data-osrc next to the (base64) src.
    let src = "![alt](pics/x.png)\n";
    let opts = RenderOptions {
        preview_edit: true,
        ..RenderOptions::preview()
    };
    let html = render_html(src, &opts).unwrap();
    assert!(html.contains(r#"data-osrc="pics/x.png""#));

    // Without edit mode there is no data-osrc.
    let plain = render_html(src, &RenderOptions::preview()).unwrap();
    assert!(!plain.contains("data-osrc"));
}
